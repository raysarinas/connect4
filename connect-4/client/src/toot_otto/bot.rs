...
impl Bot {
    ...
    fn set_rand_token(&mut self) {
        let timestamp = stdweb::web::Date::now(); // in millis
        let flag: i32 = ((timestamp % 10 as f64).floor() as i32) % 2;
        match flag {
            0 => self.set_token(Token::T),
            _ => self.set_token(Token::O),
        }
    }
    ...
    fn min_state(&mut self, depth: isize, alpha: isize, mut beta: isize) -> (isize, isize) {
        ...
        let token: Token;
        match &self.token {
            Token::O => token = Token::T,
            Token::T => token = Token::O,
        }
